...
class TestUser(TestCase):
    ...
    def test_new_post_not_authorized(self):
        response = self.client.post(
            '/new/', {
                'text': 'testpost', 'author': self.user},
            follow=True,
            msg='Не авторизованный пользователь не может создать пост')
        self.assertRedirects(
            response, '/auth/login/?next=/new/', status_code=302,
            target_status_code=200,
            msg_prefix='Неавторизованного не перенаправляет на логин',
...
class TestPost(TestCase):
    ...
    def test_new_post_on_pages(self):
        ...
        for url in urls:
            ...
            self.assertContains(
                response, self.post.text, count=None,
                status_code=200, msg_prefix='Пост не найден', html=False)
    ...
    def test_post_edit(self):
        ...
        response = self.client.post(
            f'/testuser/{post_id}/edit/', {'text': 'Modified test post'})
        urls = ['/', '/testuser/', f'/testuser/{self.post_id}/']
        for url in urls:
            response = self.client.get(url)
            self.assertContains(
                response, 'Modified test post', count=None,
                status_code=200,
                msg_prefix=f'Пост не найден ({self.post.text})', html=False)
...
class TestExceptions(TestCase):
    ...
    def test_404_error(self):
        ...
        self.assertTemplateUsed(
            ...
            msg_prefix='Не использован шаблон 404.html')
...
class TestFollowings(TestCase):
    ...
    def test_not_authorized_follow(self):
        ...
        self.assertNotContains(
            response, 'Отписаться', status_code=302,
            msg_prefix='Удалось подписаться неавторизованному')
    ...
    def test_authorized_follow_post(self):
        self.client.force_login(self.follower)
